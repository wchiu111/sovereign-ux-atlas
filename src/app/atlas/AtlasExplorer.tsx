...
useEffect(() => {
  ...
  const navigateFromSearch = useCallback((destination: AtlasSearchDestination) => {
    ...
    if (destination.type === "observatory") {
      onEnterObservatory?.();
      return;
    }
    ...
    const system = SYSTEMS.find((candidate) => candidate.id === destination.systemId);
  ...
  const openAssistSource = useCallback((source: AtlasAssistSource) => {
    ...
    const destination = getAtlasEntry(destinationId);
    const destinationSystem = SYSTEMS.find((system) =>
      system.planets.some((planet) => planet.id === destinationId),
    );
    if (!destination || !destinationSystem) return;

    const sectionIndex = source.sectionId
      ? destination.sections?.findIndex((section) => section.id === source.sectionId) ?? -1
      : -1;
    ...
    actions.openPlanet(destinationSystem.id, destination.id);
    requestAnimationFrame(() => actions.enterFocusMode(sectionIndex));
  }, [actions, activePlanetId]);
